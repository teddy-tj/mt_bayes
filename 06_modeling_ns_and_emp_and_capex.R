...
dir3 <- "../Data/final/"
dir_output <- paste0(dir3, "ns_and_emp_and_capex_control/")
# options
...
if (!file.exists(paste0(dir_output, "second_models_done.txt"))) {
  ...
  stan_model_2_1 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_rd) | industry_alternative), 
    data = modeling_data,
  ...
  stan_model_2_2 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_rd) | industry),
    data = modeling_data,
    chains = n_chains, iter = n_iter, seed = seed,
    cores = 4
  )
  ...
  stan_model_2_3 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_rd) | clusters),
    data = modeling_data,
...
if (!file.exists(paste0(dir_output, "fourth_models_done.txt"))) {
  ...
  stan_model_4_1 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | industry_alternative), 
    data = modeling_data,
    chains = n_chains, iter = n_iter, seed = seed, 
    cores = 4 
  )
  ...
  stan_model_4_2 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | industry),
    data = modeling_data,
    chains = n_chains, iter = n_iter, seed = seed,
    cores = 4
  )
  ...
  stan_model_4_3 <- stan_lmer(
    log_comp_shifted_op ~ log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | clusters),
    data = modeling_data,
    chains = n_chains, iter = n_iter, seed = seed,  # MCMC settings
    cores = 4  # Use 4 cores for parallel processing
  )
...
#     log_comp_shifted_op ~ lag(log_cum_rd) + log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | industry_alternative), 
#     data = modeling_data,
#     chains = n_chains, iter = n_iter, seed = seed, 
#     cores = 4 
#   )
...
#     log_comp_shifted_op ~ lag(log_cum_rd) + log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | industry),
#     data = modeling_data,
#     chains = n_chains, iter = n_iter, seed = seed,
#     cores = 4
#   )
...
#     log_comp_shifted_op ~ lag(log_cum_rd) + log_ns + log_emp + log_capex + (1 | company_name) + (0 + lag(log_cum_rd) | clusters),
#     data = modeling_data,
#     chains = n_chains, iter = n_iter, seed = seed,  # MCMC settings
#     cores = 4  # Use 4 cores for parallel processing
#   )
